...
export const getLikesAndDislikes = async (req, res, next) => {
  if (!req.user.isAdmin) {
    return next(errorHandler(403, 'You are not authorized to view this information'));
  }

  try {
    const post = await Post.findById(req.params.postId).populate('likes dislikes', 'username email');
